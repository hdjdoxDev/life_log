...
class LoginModel extends IScrollableModel {
  ...
  final List<LogEntry> _logs = [
    ...
    LogEntry(
        msg: "Type 'user, the_user' if you'r tired of being called 'guest'",
        id: 2,
        lastModified: now.millisecondsSinceEpoch),
    LogEntry(
        msg: "Type 'color, 0-11' to change the color of the app",
        id: 3,
        lastModified: now.millisecondsSinceEpoch),
    LogEntry(
        msg: "Type 'pass, the_pass' to login, 'guest' for the first time",
        id: 3,
        lastModified: now.millisecondsSinceEpoch),
  ];
  @protected
  ...
  void getInput(VoidCallback onLogin) async {
    ...
    var name = words[0];
    var msg = words[1];
    if (name == ISettingsApi.passName &&
        msg == await settingApi.getSetting(ISettingsApi.passName)) {
      appendLog(
          "Welcome back ${await settingApi.getSetting(ISettingsApi.userName)}!");
      Future.delayed(const Duration(seconds: 1), () => onLogin());
      ...
      appendLog("Setting username to $msg");
